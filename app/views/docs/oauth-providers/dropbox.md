...
Appwrite allows you to authenticate users using their Dropbox account through the Dropbox OAuth2 provider. OAuth authentication is a great way to reduce friction for your users and increase user conversion by simplifying the signup process. You can learn more about Appwrite's other OAuth2 providers [here](placeholder link). 

## Enabling the Dropbox provider
Before you can use Dropbox to authenticate users, you need to enable the provider in your Appwrite console.
1. Navigate to your Appwrite project
2. Navigate to **Auth** > **Settings**
3. Find and open the OAuth provider
4. In the **Dropbox OAuth2 Settings** modal, use the toggle to enable the provider

Don't close this modal, we'll need to create a Dropbox app to complete this form.

## Creating a Dropbox app for OAuth
To use Dropbox OAuth with Appwrite, you need to create a Dropbox app using a Dropbox developer account. Go to [Dropbox Developers Dashboard](https://www.dropbox.com/developers), in the [App Console](https://www.dropbox.com/developers/apps) you can create a new drobox app, first verify with your email than click on Create app button to add a new app. Choose the API, type of access you need and your app name. Enable additional users in Development users, in permissions allow profile and email access under OpenID Scopes and add **Redirect URI**, provide the **URI** found in the **Dropbox OAuth2 Settings** modal from your Appwrite console.

After you've created your Dropbox app, you can head back to your Appwrite console to complete the form in the **Dropbox OAuth2 Settings** modal.
- You can find the **App key** in your Dropbox app settings menu and provide this in the **App ID** field in the **Dropbox OAuth2 Settings** modal from the Appwrite console.
- Click on **Show App secret** to get your app's client secret.
- Copy the client secret and provide this in the **App Secret** field in the **Dropbox OAuth2 Settings** modal from the Appwrite console.

## Authenticating
You can use any of the Appwrite Client SDKs to authenticate users with their Dropbox account.

### Web
When a user calls the [Create OAuth2 Session](https://appwrite.io/docs/client/account#accountCreateOAuth2Session) endpoint in your web app, they will be taken to Dropbox's OAuth page to complete their login. 
...
// Go to Dropbox OAuth login page
account.createOAuth2Session('dropbox', '[LINK_ON_SUCCESS]', '[LINK_ON_FAILURE]');
```
...
void main() async {
    ...
    await account.createOAuth2Session(provider: 'dropbox');
...
account.createOAuth2Session(provider = "dropbox")
```
...
let success = try await account.createOAuth2Session(provider: "dropbox")
```
